...
pub fn gen_custom_type_docs<W: fmt::Write>(
    ...
    if !typ.docs().is_empty() {
        CppRst::from_markdown(
            &typ.docs().to_markdown(docs_url_gen, true),
            in_path,
            env,
            &mut class_indented,
        )?;
...
pub fn gen_method_docs<W: fmt::Write>(
    ...
    if !method.docs.is_empty() {
        CppRst::from_markdown(
            &method.docs.to_markdown(docs_url_gen, true),
            in_path,
            env,
            &mut method_indented,
        )?;
...
pub fn gen_field_docs<W: fmt::Write>(
    ...
    if !field.2.is_empty() {
        ...
        CppRst::from_markdown(
            &field.2.to_markdown(docs_url_gen, true),
            in_path,
            env,
            &mut field_indented,
        )?;
...
pub fn gen_enum_variant_docs<W: fmt::Write>(
    ...
    if !variant.2.is_empty() {
        ...
        CppRst::from_markdown(
            &variant.2.to_markdown(docs_url_gen, true),
            in_path,
            env,
            &mut enum_indented,
        )?;
