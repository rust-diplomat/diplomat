...
impl<'ccx, 'tcx: 'ccx, 'header> TyGenContext<'ccx, 'tcx, 'header> {
    ...
    fn gen_method_info(
        ...
        for param in method.params.iter() {
            ...
            if matches!(
                param.ty,
                CanBeInputType::Everywhere(Type::Slice(hir::Slice::Str(
                    _,
                    hir::StringEncoding::Utf8
                )))
            ) || matches!(
                param.ty,
                CanBeInputType::InputOnly(Type::Slice(hir::Slice::Str(
                    _,
                    hir::StringEncoding::Utf8
                )))
            ) {
            ...
            let conversions = match &param.ty {
                CanBeInputType::Everywhere(ty) => {
                    self.gen_cpp_to_c_for_type(&ty, param.name.as_str().into(), true)
                }
