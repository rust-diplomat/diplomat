...
use crate::dotnet::r#gen::method::{self, MethodInfo, PropertyInfo, StructMethodContext};
    ...
    struct_def
        ...
        .map(|field| {
            let field_type = match &field.ty {
                hir::Type::Primitive(p) => StructFieldType::Primitive(DotnetPrimitives::from(p)),
                hir::Type::Enum(enum_path) => StructFieldType::Enum(ctx.enum_name(enum_path)),
                _ => panic!("Only primitive and enum fields are supported in structs: {field:?}"),
            };
            StructField {
                name: ctx
                    .formatter
