...
mod test {
    ...
    #[test]
    fn test_method_overloading_python_keyword() {
        ...
        // "Method Info for from_ already exists but isn't a getter or setter!"
        let mut method_generated = String::default();
        context.gen_opaque_def(opaque_def, type_id, &mut method_generated);
        let root_generated = root_module.to_string();

        // Snapshot both the method bindings and root module
        // The method bindings show the actual generated C++ code with overloaded from_() methods
        insta::assert_snapshot!("method_bindings", method_generated);
        insta::assert_snapshot!("root_module", root_generated)
    }
}
