...
inline somelib::Foo somelib::Foo::new_(std::string_view x DIPLOMAT_LIFETIME_BOUND) {
    auto result = somelib::capi::Foo_new({x.data(), x.size()});
    return somelib::Foo::FromFFI(result);
}

inline somelib::Bar somelib::Foo::get_bar() const DIPLOMAT_LIFETIME_BOUND {
    auto result = somelib::capi::Foo_get_bar(this->AsFFI());
    return somelib::Bar::FromFFI(result);
}

inline somelib::Foo somelib::Foo::new_static(std::string_view x) {
...
inline somelib::Foo somelib::Foo::extract_from_fields(somelib::BorrowedFields fields DIPLOMAT_LIFETIME_BOUND) {
    auto result = somelib::capi::Foo_extract_from_fields(fields.AsFFI());
    return somelib::Foo::FromFFI(result);
}

inline somelib::Foo somelib::Foo::extract_from_bounds(somelib::BorrowedFieldsWithBounds bounds DIPLOMAT_LIFETIME_BOUND, std::string_view another_string DIPLOMAT_LIFETIME_BOUND) {
