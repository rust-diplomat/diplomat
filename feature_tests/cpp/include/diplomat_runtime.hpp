...
public:
  ...
  // Allow assignment of non-const spans to const spans
  template <typename U = std::remove_const_t<T>, typename E_ = std::enable_if_t<std::is_const_v<T>>>
  constexpr span(T *data = nullptr, size_t size = 0) 
    : data_(data), size_(size) {}
