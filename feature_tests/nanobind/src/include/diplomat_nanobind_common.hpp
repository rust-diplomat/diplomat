    ...
    public:
        ...
        bool from_python(handle src, uint8_t flags, cleanup_list* cleanup) noexcept {
            ...
            if (nb::inst_check(src)) {
                PyTypeObject *tp = Py_TYPE(src.ptr());
                const std::type_info& inf = *(nb_type_data(tp)->type);
                if (inf == typeid(U)) {
...
struct get_cb_with_lvalue_arg<std::function<Return(Args...)>> {
    ...
    static std::function<Return(Args...)> get_arg(Type f) {
        return [f](Args... args) {
            ...
            f(lvalue_to_ptr<Args>::get_value(args)...);
        };
    }
};
...
struct swap_lvalue_wrapper<Return(*)(Args...)> {
    ...
    static result get_converted_func(original f) {
        return [f](get_cb_with_lvalue_arg<Args>::Type... args) {
